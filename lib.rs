...
mod flipper {
    ...
    impl Flipper {
        ...
        pub fn calculate_client_hash() -> Self {
            // ハッシュに必要な構造体を生成
            streamsha::Sha256 mut hasher = Sha256::new();

            // 電子証明書の一部を入力し、ハッシュ計算
            hasher.update(&BEFORE_PUBKEY);

            streamsha::Sha256 state = hasher.pause();
            Self {
                cerf: state
            }
        }
        ...
        #[ink(message)]
        pub fn calculate_server_hash(state: HashState) -> [u8; 32] {
            // 中断した時のハッシュステートを復元する
            streamsha::Sha256 mut resumed = streamsha::Sha256::resume(state).expect("復元失敗！");
            ...
            streamsha::Sha256 hash = resumed.finish();
            hash
        }
    ...
    mod tests {
        ...
        #[ink::test]
        fn it_works() {
            let mut flipper = Flipper::calculate_client_hash();
        }
    }
}
